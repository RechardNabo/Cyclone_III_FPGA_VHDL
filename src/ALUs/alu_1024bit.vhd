-- 1024-bit ALU for Cyclone III FPGA
...
end entity alu_1024bit;

architecture rtl of alu_1024bit is
...
begin
    ...
    process(a, b, alu_op)
        variable sum_ext : unsigned(1024 downto 0);
        variable sub_ext : unsigned(1024 downto 0);
    begin
        res      <= (others => '0');
        carry    <= '0';
        overflow <= '0';
        ...
        case alu_op is
            ...
            when OP_ADD =>  -- A + B
                ...
                res      <= std_logic_vector(sum_ext(1023 downto 0));
                carry    <= sum_ext(1024);
                overflow <= (a(1023) and b(1023) and (not sum_ext(1023))) or
                            ((not a(1023)) and (not b(1023)) and sum_ext(1023));

            when OP_SUB =>  -- A - B
                sub_ext := ('0' & unsigned(a)) - ('0' & unsigned(b));
                res      <= std_logic_vector(sub_ext(1023 downto 0));
                carry    <= sub_ext(1024);
                overflow <= (a(1023) and (not b(1023)) and (not sub_ext(1023))) or
                            ((not a(1023)) and b(1023) and sub_ext(1023));
            ...
            when OP_INC =>  -- A + 1
                ...
                res      <= std_logic_vector(sum_ext(1023 downto 0));
                carry    <= sum_ext(1024);
                overflow <= (a(1023) and (not sum_ext(1023)));

            when OP_DEC =>  -- A - 1
                sub_ext := ('0' & unsigned(a)) - 1;
                res      <= std_logic_vector(sub_ext(1023 downto 0));
                carry    <= sub_ext(1024);
                overflow <= ((not a(1023)) and sub_ext(1023));
